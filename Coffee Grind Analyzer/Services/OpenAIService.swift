...
class OpenAIService {
    ...
    private let logger = Logger(subsystem: "com.nateking.GrindLab", category: "OpenAIService")

    private init() {
        secureStore.migrateString(from: keychainKey, to: keychainKey)
    }

    private func apiKey() -> String? {
        do {
            return try secureStore.string(for: keychainKey)
        } catch {
            logger.error("Failed to read API key from Keychain: \(error.localizedDescription, privacy: .public)")
            return nil
        }
    }
    ...
    enum OpenAIError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .httpError(let statusCode):
                return "OpenAI HTTP error: \(statusCode)"
